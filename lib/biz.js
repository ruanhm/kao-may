            ...
            if (_.isArray(p.params) && p.params.length > 0) {
                for (let bizParameter of p.params) {
                    if (bizParameter.isLoop) {
                        for (let k of allKey.filter((ele) => { ele.indexOf(bizParameter.param) > -1 })) {
                            psMap.set(k, {
...
class BizQuery extends Biz {
    ...
    async execute(ps) {
        ...
        try {
            ...
            runPs.push({ name: 'beginIndex', type: TYPES.Int, value: 1 })
            runPs.push({ name: 'endIndex', type: TYPES.Int, value: 50 })
            var pormises = [];
            //执行sql
            for (let sql of p.sqls) {
                let runSql = p._pagingGenerator(sql.sql);
            ...
            try {
                if ((p.countSql && p.countSql.trim().length > 0) || isHasCountTag) {
                    if ((!p.countSql || p.countSql.trim().length == 0) && isHasCountTag) {
                        let runSql = p._countGenerator(p.sqls[0].sql);
                        rs.Data.push(await dbhelper.query(runSql, runPs));
                    }
                }
            }
...
class BizOperation extends Biz {
    ...
    async execute(ps) {
        ...
        try {
            ...
            var outputs = p._findOutputParamaters();
            var psMap = p._getSqlParamaters(ps);
    ...
    _getSqlRunParamaters(psMap, loopIndex = 0, loopField) {
        ...
        if (p.params && p.params.length > 0) {
            var keyTag = '';
            if (loopField && loopField.trim() > 0) {
                var tempStr = '[' + loopField + '][' + loopIndex + ']';
                keyTag = [...psMap.keys].find((ele) => { ele.indexOf(tempStr) > -1 });
                keyTag = keyTag ? keyTag.replace(tempStr) : '';
            }
            for (let param of p.params) {
                if (psMap.has(param.param) || psMap.has(keyTag + '[' + loopIndex + '][' + param.param + ']')) {
    ...
    _getSqlRunCount(psMap, loopField) {
        return loopField && loopField.trim().length > 0 ?
            [...psMap.keys()].filter((ele) => { ele.indexOf('[' + loopField + ']') > -1 }).length : 1;
    }
    ...
    _findOutputParamaters() {
        ...
        var psArr = [];
        if (_.isArray(p.params) && p.params.length > 0) {
            psArr = p.params.filter((ele) => { ele.isReturn });
        }
        return psArr;
    }
    _findLoopParamaters() {
        ...
        var psArr = [];
        if (_.isArray(p.params) && p.params.length > 0) {
            psArr = p.params.filter((ele) => { ele.isLoop });
        }
        return psArr;
    }
    _checkFieldIsLoop(checkField) {
        var loopParams = this._findLoopParamaters();
        return loopParams.filter((ele) => { ele.checkField === checkField }).length > 0;
    }
}
